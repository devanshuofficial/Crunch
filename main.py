...
Cloud.config( 
  cloud_name = "dnxcv2qoe", 
  api_key = "783696871659819", 
  api_secret = "oc5p0xHMxkwC6_HlWXVgh_bEWtY"
)
...
def sendimage(data):
    ...
    if Chats.query.filter_by(id = session['imageid']).count() == 1:
        ...
        join_room(room.name)
        emit('receiveMessage', data, broadcast= True, to=room.name)
        # emit('receiveimage', data, to=session['name'] )
...
def handle_message(data):
    ...
    if user.workspace_list:
        ...
        join_room(room.name)
    emit('receiveMessage', {"msg": data['data'], "wid":"1", "channel_d":"2"})
...
def handle_createChannel(data):
    ...
    emit('createChannelJS',data, to=room.name, broadcast= True)
...
def sendMessages(data):
    ...
    for c in chats:
        ...
        i = i + 1
    emit('receiveMessageJS', {"chats":ch, "channel_id":data['channel_id'], "name":channel.name}, broadcast= True, to=room.name)
...
def addWorkspace(data):
    ...
    if Workspace.query.filter_by(name = data['name'], joining_code=data['code']).count() == 1:
        ...
        if user.workspace_list:
            ...
            wid = room.id
            if wid in wlist:
                emit('error', {"msg":"You have already joined the workspace!", "username":data['username']}, to=room.name)
            else:
                user.workspace_list = user.workspace_list + str(room.id) + " "
                emit('workspaceJoined', {"wid": room.id, "username":data['username'], "name": room.name,}, to=room.name)
        else:
            room = Workspace.query.filter_by(name = data['name'],).first()
            user.workspace_list = str(room.id) +" "
            emit('workspaceJoined', {"wid": room.id, "name": room.name, "username":data['username']}, to=room.name)
    db.session.commit()
